...
module.exports = function (grunt) {
	...
	grunt.registerMultiTask('jekyll', 'This triggers the `jekyll` command.', function () {
		...
		function runJekyll (next) {
		    ...
		    spawnTest = spawn('jekyll', ['serve', '--source ./test/app', '--destination ./test/serve', '--watch']);

			spawnTest.stdout.on('data', function (data) {
			  console.log('' + data);
			});

			spawnTest.stderr.on('data', function (data) {
			  console.log('' + data);
			});

			spawnTest.on('close', function (code) {
